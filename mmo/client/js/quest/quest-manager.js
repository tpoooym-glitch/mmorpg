...
function objectiveMatches(o,type,target){return o.type!==type?false:o.target==null||o.target===target}
...
export function createQuestManager(state,catalog=[]){
  ...
  const manager={
    active:Array.isArray(state.player.quests)?state.player.quests:[],catalog:Array.isArray(catalog)?catalog:[],max,
    ...
    canTurnIn(id,npcId){npcId=npcId||state.ui?.activeNpc||'mira';const q=this.find(id);return !!q&&q.status==='complete'&&q.giverNpc===npcId},
    turnIn(id,npcId){npcId=npcId||state.ui?.activeNpc||'mira';const index=this.active.findIndex(q=>q.id===id&&q.status==='complete'&&q.giverNpc===npcId);if(index<0)return null;const q=this.active.splice(index,1)[0],rewards=q.rewards||{};state.player.quests=this.active;state.player.gold=Math.max(0,(state.player.gold||0)+(rewards.gold||0));applyXp(state.player,rewards.exp||0);state.player.pendingQuestRewards=Array.isArray(state.player.pendingQuestRewards)?state.player.pendingQuestRewards:[];for(const item of rewards.items||[])if(item?.id&&item.qty>0)state.player.pendingQuestRewards.push({...item,sourceQuest:q.id});return clone(q)},
    abandon(id){const index=this.active.findIndex(q=>q.id===id);if(index<0)return false;this.active.splice(index,1);state.player.quests=this.active;return true},snapshot(){return clone(this.active)}
  };state.player.quests=manager.active;state.questManager=manager;return manager
}
