...
export function speed(state){const p=state.player,h=p.hitbox||{offsetX:0,offsetY:0},x=p.x+(h.offsetX||0),y=p.y+(h.offsetY||0),t=terrainAt(state,x,y);return p.speed*(state.zone?.movement?.[t+'Multiplier']??1)}
