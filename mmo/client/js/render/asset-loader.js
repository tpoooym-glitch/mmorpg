...
const playerSources={warriorWalk:'assets/player/warrior-walk.png',warriorAttack:'assets/player/warrior-attack.png'};
...
export function loadAssets(){const assets={};const pending=[];for(const [name,src] of Object.entries(sources)){pending.push(loadImage(src).then(image=>{assets[name]=image}).catch(error=>{assets[name]=null;console.warn(error.message)}))}for(const [name,spec] of Object.entries(encodedSources)){pending.push(loadEncodedImage(spec).then(image=>{assets[name]=image}).catch(error=>{assets[name]=null;console.warn(`Asset ${name} failed: ${error.message}`)}))}for(const [name,src] of Object.entries(playerSources)){pending.push(loadImage(src).then(image=>{assets[name]=image}).catch(error=>{assets[name]=null;console.warn(error.message)}))}return{assets,ready:Promise.all(pending)}}
