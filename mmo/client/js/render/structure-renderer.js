import {drawGridSprite,getAlphaBounds} from './sprite-atlas.js';
const defs={
...
function drawImageBounds(ctx,img,b,target){if(!img?.complete||!img.naturalWidth||!b)return false;const pad=4,sx=Math.max(0,b.x-pad),sy=Math.max(0,b.y-pad),sw=Math.min(img.naturalWidth-sx,b.w+pad*2),sh=Math.min(img.naturalHeight-sy,b.h+pad*2),scale=Math.min(target.w/sw,target.h/sh),dw=Math.max(1,Math.round(sw*scale)),dh=Math.max(1,Math.round(sh*scale));ctx.drawImage(img,sx,sy,sw,sh,Math.round(-dw/2),Math.round(-dh/2),dw,dh);return true}
...
function drawOne(ctx,state,s){const d=defs[s.type];if(!d)return;const x=s.x-state.cam.x,y=s.y-state.cam.y;ctx.save();ctx.translate(Math.round(x),Math.round(y));ctx.imageSmoothingEnabled=false;let drawn=false;if(d.src){drawn=drawImageBounds(ctx,getImage(d.src),getAlphaBounds(getImage(d.src)),{w:s.w,h:s.h})}else if(d.asset){const img=state.assets?.[d.asset];if(img?.complete&&img.naturalWidth){drawn=d.grid?drawGridSprite(ctx,img,s.assetIndex||0,d.grid[0],d.grid[1],{w:s.w,h:s.h}):false}}if(!drawn){if(s.type==='well')drawWellFallback(ctx,s);else drawFallback(ctx,s)}ctx.restore()}
