const defs={
  'adventurer-house':{asset:'assets/buildings/adventurer-guild.png',tone:'#8f5b37'},
  'villager-house':{asset:'assets/buildings/villager-house.png',tone:'#a66a3d'},
  blacksmith:{asset:'assets/buildings/blacksmith-shop.png',tone:'#70452f'},
  'general-store':{asset:'assets/buildings/general-store.png',tone:'#9b6338'}
};
...
function drawFallback(ctx,s){
  ...
  if(s.type==='blacksmith'){
    ctx.fillStyle='#777';ctx.fillRect(w*.30,-h*.05,w*.24,h*.32);ctx.strokeRect(w*.30,-h*.05,w*.24,h*.32);
    ctx.fillStyle='#ff9b22';ctx.fillRect(w*.36,h*.05,w*.12,h*.14);
  }
  if(s.type==='general-store'){
    ctx.fillStyle='#d6a24a';ctx.fillRect(-w*.28,h*.02,w*.56,h*.12);ctx.strokeRect(-w*.28,h*.02,w*.56,h*.12);
  }
