...
function drawOne(ctx,state,s){const d=defs[s.type];if(!d)return;const x=s.x-state.cam.x,y=s.y-state.cam.y;ctx.save();ctx.translate(Math.round(x),Math.round(y));ctx.imageSmoothingEnabled=false;let ok=false;if(d.src)ok=standalone(ctx,image(d.src),s);else if(d.source)ok=drawSourceSprite(ctx,image(locationSrc),d.source,{w:s.w,h:s.h});else if(d.well)ok=well(ctx,s);else if(d.compositeFarm)ok=farm(ctx,s);else if(d.fence)ok=fence(ctx,s);else if(d.gate)ok=gate(ctx,s);else if(d.spawn)ok=spawnMarker(ctx,s);else if(d.asset)ok=grid(ctx,state.assets?.[d.asset],s.assetIndex||0,d.grid[0],d.grid[1],s);if(!ok&&!d.spawn&&!d.asset&&!d.source&&!d.well)fallback(ctx,s);ctx.restore()}
