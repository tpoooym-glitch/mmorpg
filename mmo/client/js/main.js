...
function loop(t){const dt=Math.min(.05,(t-last)/1000);last=t;move(state,input.keys,dt);updateCombat(state,dt);updateCamera();drawScene(ctx,state,assets);drawStructures(ctx,state);updateHud(state);requestAnimationFrame(loop)}
