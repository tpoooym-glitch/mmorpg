...
async function start(){const loaded=await loadZone();state.zone=loaded.zone;state.error=loaded.error;state.player.x=state.zone.spawn.x;state.player.y=state.zone.spawn.y;state.player.spawnX=state.zone.spawn.x;state.player.spawnY=state.zone.spawn.y;state.assets=assets;updateCamera();state.decor=generateDecor(state);state.mobs=createMobs();await loadSkills();requestAnimationFrame(loop);ready.catch(error=>{state.error=`Asset startup failed: ${error?.message||error}`;console.error(error)})}
function loop(t){const dt=Math.min(.05,(t-last)/1000);last=t;move(state,input.keys,dt);updateCombat(state,dt);updateCamera();drawScene(ctx,state,assets);drawRoadOverlay(ctx,state);drawStructures(ctx,state);drawActorsOverlay(ctx,state,assets);updateHud(state);requestAnimationFrame(loop)}
