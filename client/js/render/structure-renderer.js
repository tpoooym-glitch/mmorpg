...
const defs={
  ...
  'village-fence':{villageFence:true},
  'yard-fence':{yardFence:true},
...
function gate(ctx,s){const img=image(fenceSrc);if(!img.complete||!img.naturalWidth)return false;const src={x:84,y:608,w:460,h:336};const vertical=s.gateDir==='vertical';const target=vertical?{w:s.h,h:s.w}:{w:s.w,h:s.h};return drawSourceSprite(ctx,img,src,target,vertical?Math.PI/2:0)}
function fencePost(ctx,x,y,scale=1){ctx.save();ctx.fillStyle='#6b4329';ctx.strokeStyle='#2f2118';ctx.lineWidth=2;ctx.fillRect(x-5*scale,y-8*scale,10*scale,16*scale);ctx.strokeRect(x-5*scale,y-8*scale,10*scale,16*scale);ctx.fillStyle='#9a673d';ctx.fillRect(x-4*scale,y-7*scale,3*scale,13*scale);ctx.restore()}
function fenceRun(ctx,x1,y1,x2,y2,gapStart=0,gapEnd=0){const dx=x2-x1,dy=y2-y1,len=Math.hypot(dx,dy)||1,ux=dx/len,uy=dy/len,px=-uy,py=ux,postStep=74,start=Math.max(gapEnd,0),end=len-Math.max(gapStart,0);if(end<start)return;ctx.save();ctx.strokeStyle='#3a281c';ctx.lineWidth=8;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x1+ux*start,y1+uy*start);ctx.lineTo(x1+ux*end,y1+uy*end);ctx.stroke();ctx.strokeStyle='#8a5a34';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(x1+ux*start+px*5,y1+uy*start+py*5);ctx.lineTo(x1+ux*end+px*5,y1+uy*end+py*5);ctx.stroke();ctx.beginPath();ctx.moveTo(x1+ux*start-px*7,y1+uy*start-py*7);ctx.lineTo(x1+ux*end-px*7,y1+uy*end-py*7);ctx.stroke();for(let d=start;d<=end+.1;d+=postStep)fencePost(ctx,x1+ux*d,y1+uy*d,1);fencePost(ctx,x1+ux*end,y1+uy*end,1);ctx.restore()}
function loopFence(ctx,s,gaps={}){const hw=s.w/2,hh=s.h/2;fenceRun(ctx,-hw,-hh,0,-hh,0,hw-(gaps.northGap||0)/2);fenceRun(ctx,0,-hh,hw,-hh,hw+(gaps.northGap||0)/2,0);fenceRun(ctx,-hw,hh,0,hh,0,hw-(gaps.southGap||0)/2);fenceRun(ctx,0,hh,hw,hh,hw+(gaps.southGap||0)/2,0);fenceRun(ctx,-hw,-hh,-hw,0,0,hh-(gaps.westGap||0)/2);fenceRun(ctx,-hw,0,-hw,hh,hh+(gaps.westGap||0)/2,0);fenceRun(ctx,hw,-hh,hw,0,0,hh-(gaps.eastGap||0)/2);fenceRun(ctx,hw,0,hw,hh,hh+(gaps.eastGap||0)/2,0)}
function villageFence(ctx,s){loopFence(ctx,s,{northGap:s.gateGapN||180,southGap:s.gateGapS||180,westGap:s.gateGapW||160,eastGap:s.gateGapE||160});return true}
function yardFence(ctx,s){const gap=s.gateWidth||150,side=s.gateSide||'south',hw=s.w/2,hh=s.h/2;if(side==='south'){fenceRun(ctx,-hw,-hh,hw,-hh);fenceRun(ctx,-hw,-hh,-hw,hh);fenceRun(ctx,hw,-hh,hw,hh);fenceRun(ctx,-hw,hh,-gap/2,hh);fenceRun(ctx,gap/2,hh,hw,hh)}else if(side==='north'){fenceRun(ctx,-hw,hh,hw,hh);fenceRun(ctx,-hw,-hh,-hw,hh);fenceRun(ctx,hw,-hh,hw,hh);fenceRun(ctx,-hw,-hh,-gap/2,-hh);fenceRun(ctx,gap/2,-hh,hw,-hh)}else if(side==='east'){fenceRun(ctx,-hw,-hh,hw,-hh);fenceRun(ctx,-hw,-hh,-hw,hh);fenceRun(ctx,-hw,hh,hw,hh);fenceRun(ctx,hw,-hh,hw,-gap/2);fenceRun(ctx,hw,gap/2,hw,hh)}else{fenceRun(ctx,-hw,-hh,hw,-hh);fenceRun(ctx,-hw,-hh,-hw,hh);fenceRun(ctx,-hw,hh,hw,hh);fenceRun(ctx,-hw,-hh,-hw,-gap/2);fenceRun(ctx,-hw,gap/2,-hw,hh)}return true}
...
function drawOne(ctx,state,s){const d=defs[s.type];if(!d)return;const x=s.x-state.cam.x,y=s.y-state.cam.y;ctx.save();ctx.translate(Math.round(x),Math.round(y));ctx.imageSmoothingEnabled=false;let ok=false;if(d.src)ok=standalone(ctx,image(d.src),s);else if(d.fountain)ok=fountain(ctx,s,d);else if(d.source){const img=image(locationSrc);ok=drawSourceSprite(ctx,img,d.source,{w:s.w,h:s.h})}else if(d.well)ok=well(ctx,s);else if(d.compositeFarm)ok=farm(ctx,s);else if(d.fence)ok=fence(ctx,s);else if(d.gate)ok=gate(ctx,s);else if(d.villageFence)ok=villageFence(ctx,s);else if(d.yardFence)ok=yardFence(ctx,s);else if(d.spawn)ok=spawnMarker(ctx,s);else if(d.asset)ok=grid(ctx,state.assets?.[d.asset],s.assetIndex||0,d.grid[0],d.grid[1],s);if(!ok&&!d.spawn&&!d.asset&&!d.source&&!d.well&&!d.fountain)fallback(ctx,s);ctx.restore()}
